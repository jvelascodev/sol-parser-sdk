...
Sol-parser-sdk 提供**两种可插拔的解析器实现**，可在编译时选择：
...
| 解析器类型 | 性能 | 代码复杂度 | 类型安全 | 内存分配 |
|-------------|-------------|-----------------|-------------|-------------------|
| **Borsh**（默认） | 95%+ | 低（10 行） | ✅ 完整 | 最小化 |
| **零拷贝** | 100% | 中等（30 行） | ⚠️ 手动 | 零分配 |

**建议：** 大多数场景下使用 **Borsh 解析器**（默认）。仅当性能分析显示解析是瓶颈时，才切换到**零拷贝解析器**。

---

#### 🔄 切换解析器

你可以通过更新 `Cargo.toml` 轻松切换解析器：
